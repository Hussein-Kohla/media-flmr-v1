...
import {
  Plus, ArrowLeft, Mail, Phone, MapPin, Building, Globe,
  FileText, Edit2, Trash2, AlertCircle, Wallet,
  TrendingUp, CheckCircle2, Clock, Archive, MessageCircle,
  ExternalLink, CreditCard, UserCheck
} from "lucide-react";
...
export default function ClientDetailClient() {
  ...
  const totalPaid = projects.reduce((sum, p) => p.paymentStatus === "paid" ? sum + p.budget : p.paymentStatus === "partial" ? sum + (p.budget * 0.5) : sum, 0);
  ...
  return (
    ...
    <span className="font-bold text-green-500">${totalPaid.toLocaleString()}</span>
    </div>
    <div className="flex justify-between items-center">
    <span className="text-sm text-muted-foreground">Pending</span>
    <span className="font-bold text-orange-500">${pendingAmount.toLocaleString()}</span>
    </div>
    <div className="pt-4 border-t border-[var(--color-border)] flex items-center gap-3">
    <CreditCard className="h-4 w-4 text-muted-foreground" />
    <span className="text-xs text-muted-foreground italic">Payment methods tracked per project</span>
    </div>
    </div>
    </div>
    ...
    {nextDeadline ? new Date(nextDeadline).toLocaleDateString() : "No deadline"}
    </span>
    </div>
    </div>
    </div>
    ...
    onClick={() => {
      const newNotes = prompt("Update client notes:", client.notes || "");
      if (newNotes !== null) {
        updateClient({ id: clientId as Id<"clients">, notes: newNotes });
      }
    }}
    >
