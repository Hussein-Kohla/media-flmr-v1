...
import {
  Users, Search, LayoutGrid, List, Plus, MoreVertical, Mail,
  Building, Phone, Globe, FileText, MessageSquare, Wallet, Package, Pencil, ArrowRight, UserCircle, X
} from "lucide-react";
...
const formatCurrency = (amt?: number) =>
    new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(amt || 0);
...
function ClientModal({ open, onClose, clientToEdit }: { open: boolean; onClose: () => void; clientToEdit?: any }) {
  ...
  useEffect(() => {
    if (clientToEdit) {
      ...
      setFormData({ name: "", company: "", email: "", phone: "", waPhone: "", details: "" });
    }
  }, [clientToEdit, open]);
      ...
      <DialogContent className="sm:max-w-[600px] bg-card border-2 border-[var(--color-border)] p-8 rounded-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase tracking-widest opacity-50">Company / Brand</Label>
              <Input 
                className="h-12 bg-muted/30 border-white/5 rounded-xl font-medium focus:border-[var(--color-brand)] transition-all"
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase tracking-widest opacity-50">WhatsApp</Label>
              <Input 
                className="h-12 bg-muted/30 border-white/5 rounded-xl font-medium focus:border-[var(--color-brand)] transition-all"
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-widest opacity-50">Additional Details</Label>
            <textarea 
          ...
          <DialogFooter className="pt-4">
            ...
            <Button type="submit" className="font-bold luxury-button luxury-button-primary h-12 px-8">
...
// ─── Client Detail Modal ──────────────────────────────────────────────────
function ClientDetailModal({ open, onClose, clientId }: { open: boolean; onClose: () => void; clientId: string | null }) {
  return (
    <Dialog open={open} onOpenChange={onClose}>
    <DialogContent
    ...
    <DialogDescription className="sr-only">View and manage detailed information for the selected client.</DialogDescription>
    </DialogHeader>
...
// ─── Avatar Modal ──────────────────────────────────────────────────────
function AvatarModal({ open, onClose, client, onSave }: { open: boolean; onClose: () => void; client: any; onSave: (avatarUrl: string) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] rounded-2xl border-2 border-[var(--color-border)] bg-card p-6">
        <DialogHeader>
          <DialogTitle className="text-2xl font-heading font-extrabold">Profile Picture</DialogTitle>
          <DialogDescription>
        ...
        <div className="grid grid-cols-3 gap-4 py-8">
          {AVATARS.map((url) => (
            ...
            className={cn(
              ...
              selected === url ? "border-[var(--color-brand)] shadow-lg" : "border-transparent bg-muted/50"
            )}
            >
          ...
          <label className={cn(
            ...
            selected === "custom" && "border-[var(--color-brand)] border-solid bg-muted/50"
          )}>
          <input type="file" className="hidden" accept="image/*" onChange={handleFileUpload} />
          {customFileUrl ? (
            <img src={customFileUrl} alt="Custom upload" className="w-full h-full object-cover" />
          ) : (
            <>
        ...
        <DialogFooter className="gap-3">
          ...
          <Button onClick={handleSave} className="font-bold luxury-button luxury-button-primary">Save Changes</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function ClientCard({ client, index, onEdit, onViewDetails, onUpdateAvatar }: { client: any; index: number; onEdit: (c: any) => void; onViewDetails: (id: string) => void; onUpdateAvatar: (c: any) => void }) {
  const stats = client.stats || { totalBudget: 0, activeDeliverablesCount: 0, lastActivityDate: "" };
  ...
  return (
    ...
    <span className="truncate">{client.company || "Individual Client"}</span>
    </p>
    </div>
    ...
    <span className="font-bold text-foreground">Total: {formatCurrency(stats.totalBudget)}</span>
    </div>
...
function ClientRow({ client, index, onViewDetails }: { client: any; index: number; onViewDetails: (id: string) => void }) {
  return (
    <div
    onClick={() => onViewDetails(client._id)}
    ...
    <div className="h-10 w-10 shrink-0 rounded-full overflow-hidden bg-muted border border-[var(--color-border)] flex items-center justify-center font-black text-sm"
    style={{ background: `var(--color-brand)15`, color: `var(--color-brand)` }}>
    {client.avatar ? (
      <img
      src={client.avatar}
      alt={client.name}
    ...
    <span className="font-heading font-bold text-lg leading-none">{client.name}</span>
    <span className="text-xs text-muted-foreground">{client.company || "Client"}</span>
    </div>
    </div>
...
export default function ClientsPage() {
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter(
      c => {
        ...
        if (activeTab === "active") matchesTab = c.status === "active" || c.status === "done" || !c.status;
  ...
  if (clients === undefined) {
    return (
      ...
      {[...Array(6)].map((_, i) => (
        <div key={i} className="h-80 bg-muted animate-pulse rounded-xl border-2 border-[var(--color-border)]" />
      ))}
      </div>
      </div>
    );
        ...
        <div className="relative z-10 space-y-12">
          <div className="sticky top-0 z-20 bg-[var(--color-surface-0)] pt-2 pb-6 -mx-4 px-4">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground font-medium max-w-md text-sm">
                  A curated gallery of professional relationships and strategic partnerships.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <div className="flex items-center gap-1 bg-card border-2 border-[var(--color-border)] p-1 rounded-lg">
                  <Button
                    ...
                    className={cn("h-8 w-8 p-0", view === "grid" && "shadow-[2px_2px_0px_0px_var(--color-border)]")}
                    title="Grid view"
                  >
                  ...
                  <Button
                    ...
                    className={cn("h-8 w-8 p-0", view === "list" && "shadow-sm")}
                    title="List view"
                  >
            ...
            <div className="flex items-center gap-2 bg-black/40 p-2 rounded-full w-fit border border-white/10 shadow-inner mt-4">
              ...
              >
                Active Clients
                {activeTab === "active" && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full shadow-glow" />}
              </button>
          ...
          >
            {filteredClients.map((client, i) =>
              view === "grid" ? (
                <ClientCard key={client._id} client={client} index={i} onEdit={openEditModal} onViewDetails={openDetailModal} onUpdateAvatar={openAvatarModal} />
              ) : (
                <ClientRow key={client._id} client={client} index={i} onViewDetails={openDetailModal} />
              )
            )}
          </div>
          ...
          {filteredClients.length === 0 && (
            <div className="py-20 text-center text-muted-foreground font-bold">
              {search ? `No match found for "${search}"` : "No clients found in this category."}
            </div>
          )}
        </div>
      </div>

      <ClientModal open={showModal} onClose={() => setShowModal(false)} clientToEdit={editingClient} />
      <ClientDetailModal open={showDetailModal} onClose={() => setShowDetailModal(false)} clientId={selectedClientId || ""} />
      <AvatarModal open={showAvatarModal} onClose={() => setShowAvatarModal(false)} client={avatarClient} onSave={handleSaveAvatar} />
    </>
  );
}
