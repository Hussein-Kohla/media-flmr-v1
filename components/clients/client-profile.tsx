...
export function ClientProfile({ client }: ClientProfileProps) {
  const proofImageUrl = useQuery(api.storage.getFileUrl, { storageId: client.proofImageId || "" });
  ...
  return (
    ...
    {proofImageUrl ? (
      <Image src={proofImageUrl} alt={client.name} className="h-full w-full object-cover" fill />
    ) : (
    ...
    <p className="text-muted-foreground">{client.company || "No company specified"}</p>
    </div>
    </div>
